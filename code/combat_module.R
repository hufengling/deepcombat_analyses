...
vanilla_vae <- nn_module(
  ...
  encode_decode = function(torch_ds, raw_means, raw_sds, 
                           ...
                           resids_correct = c("scale", "combat", "none"),
                           ref_batch = NULL, mean_only = FALSE,
                           verbose = FALSE) {
    ## Check parameters
    resids_correct <- match.arg(resids_correct, c("scale", "combat", "none"))
    ...
    if (resids_correct == "combat") {
      if (verbose) {
        cat("Harmonizing autoencoder residuals using ComBat\n")
      }
      if (use_covariates[2]) {
        feat_resids <- neuroCombat(dat = t(as.matrix(feat_resids)),
                                   batch = as.matrix(torch_ds$batch),
                                   mod = as.matrix(torch_ds$covariates),
                                   ref.batch = ref_batch,
    ...
    if (resids_correct == "scale") {
      ...
      feat_resids <- torch_tensor(feat_resids_mat)
    }
    if (resids_correct == "none") {
      if (verbose) {
