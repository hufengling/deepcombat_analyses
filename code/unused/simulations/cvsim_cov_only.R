##### Covariance only
# Site effect in covariance only
# Covariate effect in covariance only
library(reshape2)
library(mvtnorm)
library(invgamma)
...
for (bf in 1:3) {
  for (cf in 1:3) {
    ...
    all_sim[[paste(bf, cf, sep = "_")]] <- list(site.AUC = site_all, 
                                                dx.AUC = dx_all, 
                                                corr.dist = corr_all,
                                                cov.dist = cov_all, 
...
save(all_sim, file = "sim_cov_only.Rdata")