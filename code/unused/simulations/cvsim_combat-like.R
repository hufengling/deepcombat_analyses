##### ComBat-like:
# Site effect in mean/variance
# Covariate effect in mean only
library(reshape2)
library(mvtnorm)
library(invgamma)
...
s <- 1 # gamma
d <- 1 # delta
c <- 4 # covariance site effect
b <- 1 # beta mean effect
bv <- 4 # beta var effect
bc <- 4 # beta covariance effect

# store results as all_sim$sf_bf
# where sf is site effect level and bf is covariate effect level
all_sim <- list()
...
for (sf in 1:3) {
  for (bf in 1:3) {
    ...
    all_sim[[paste(sf, bf, sep = "_")]] <- list(site.AUC = site_all, 
                                                dx.AUC = dx_all, 
                                                corr.dist = corr_all,
                                                cov.dist = cov_all, 
...
save(all_sim, file = "sim_combat-like.Rdata")