...
data_list <- list(raw = raw,
                  combat = input_list$data_raw,
                  covbat = covbat,
                  vae_recon = as.matrix(output$feat_recon),
                  vae_recon_new = as.matrix(vae_recon_new),
...
umap_plotter <- function(data_list, covariates, n_neighbors, n_epochs) {
  ...
  for (i in 1:length(data_list)) {
    ...
    subid <- ggplot(layout_cov) + geom_point(aes(`1`, `2`, color = subid, size = 0.1)) +
      theme(legend.position = "none")
    age <- ggplot(layout_cov) + geom_point(aes(`1`, `2`, color = AGE, size = 0.1))
    sex <- ggplot(layout_cov) + geom_point(aes(`1`, `2`, color = SEX, size = 0.1))
    diagnosis <- ggplot(layout_cov) + geom_point(aes(`1`, `2`, color = DIAGNOSIS, size = 0.1))
    random <- ggplot(layout_cov) + geom_point(aes(`1`, `2`, color = Random, size = 0.1))
    manufac <- ggplot(layout_cov) + geom_point(aes(`1`, `2`, color = manufac, size = 0.1))
...
umap_list <- umap_plotter(data_list, covariates, n_neighbors = 10, n_epochs = 80); umap_list
```

## Plot latent space of SCVI
```{r}
col <- covariates$SEX
pairs(data_list$combat_latent[, 1:5], col = col, lower.panel = NULL)
