...
final_visits_covariates <- final_visits_covariates %>% mutate(manufac = Manufacturer)
final_visits_raw <- raw[final_visits_covariates$X.1, ]

cov <- model.matrix(~ SEX + DIAGNOSIS + AGE, final_visits_covariates)[, -1]
batch <- as.matrix(model.matrix(~ manufac, final_visits_covariates)[, -1])

files_list <- lapply(list.files("./data/for_paper", 
                                full.names = TRUE, pattern = "*.csv"), 
                     read_csv)

combat_harmonized <- neuroCombat(dat = t(final_visits_raw), batch = final_visits_covariates$manufac, mod = cov)$dat.combat %>% t()
covbat_harmonized <- covbat(dat = t(final_visits_raw), bat = final_visits_covariates$manufac, mod = cov)$dat.covbat %>% t()

setup <- deepcombat_setup(~ AGE + SEX + DIAGNOSIS, ~ manufac, final_visits_raw, final_visits_covariates)
...
```{r}
write.csv(combat_harmonized, "./data/revisions/combat.csv")
write.csv(covbat_harmonized, "./data/revisions/covbat.csv")
write.csv(harmonized$harmonized, "./data/revisions/deepcombat_01.csv")
```
